...
const cover3: CoverageData = {
  ...
  diagram: {
    players: [
      ...
      { id: 'LOLB', label: 'LB', role: 'linebacker', x: 16, y: 42,
        coverageZone: { type: 'flat', rect: { x: 0, y: 32, width: 22, height: 18 } } },
      { id: 'LILB', label: 'LB', role: 'linebacker', x: 38, y: 43,
        coverageZone: { type: 'hook-curl', rect: { x: 22, y: 32, width: 28, height: 18 } } },
      { id: 'RILB', label: 'LB', role: 'linebacker', x: 62, y: 43,
        coverageZone: { type: 'hook-curl', rect: { x: 50, y: 32, width: 28, height: 18 } } },
      { id: 'ROLB', label: 'LB', role: 'linebacker', x: 84, y: 42,
        coverageZone: { type: 'flat', rect: { x: 78, y: 32, width: 22, height: 18 } } },
      // FS single high center
      { id: 'FS', label: 'FS', role: 'safety', x: 50, y: 17,
        coverageZone: { type: 'deep-middle-third', rect: { x: 33, y: 5, width: 34, height: 33 } } },
      // CBs off coverage — rotate to deep thirds
      { id: 'LCB', label: 'CB', role: 'cornerback', x: 8, y: 47,
        coverageZone: { type: 'deep-third', rect: { x: 0, y: 5, width: 33, height: 33 } } },
      { id: 'RCB', label: 'CB', role: 'cornerback', x: 92, y: 47,
        coverageZone: { type: 'deep-third', rect: { x: 67, y: 5, width: 33, height: 33 } } },
    ],
    movementArrows: [
      { fromPlayerId: 'LCB', toX: 10, toY: 23, style: 'zone-drop' },
      { fromPlayerId: 'RCB', toX: 90, toY: 23, style: 'zone-drop' },
    ],
    annotations: [
      { x: 10, y: 22, text: 'Deep 1/3', style: 'zone-label' },
      { x: 50, y: 22, text: 'Deep 1/3', style: 'zone-label' },
      { x: 90, y: 22, text: 'Deep 1/3', style: 'zone-label' },
      { x: 18, y: 6, text: '⚠ Corner route gap', style: 'warning' },
